...
o.datatype = "port"
o.optional = false
o.description = '<span class="ag-web-open-wrap"><button type="button" class="ag-web-open-btn" onclick="window.open(\'http://\'+window.location.hostname+\':' ..
	httpport .. '/\')">' .. translate("Open AdGuard Home Web UI") .. " : " .. httpport .. "</button></span>"

-- DNS Port
local port = luci.sys.exec("awk '/^dns:/ {found_dns=1} found_dns && /^  port:/ {print $2; exit}' " .. configpath .. " 2>/dev/null")
if port == "" then
	port = "?"
end
-- Update
